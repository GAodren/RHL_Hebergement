...
export default function EstimationForm({ initialState }) {
  ...
  const [initialCommission, setInitialCommission] = useState(initialState?.commission || 0);
    ...
    const loadLatestData = async () => {
      if (currentEstimationId.current && initialState?.estimationId) {
        ...
        if (estimation) {
          ...
          setInitialCommission(estimation.commission || 0);
        }
      ...
      <div className="space-y-6">
        ...
        <div
          className={`space-y-6 overflow-hidden transition-all duration-300 ease-in-out ${
            formData.categorie ? 'max-h-[1200px] opacity-100' : 'max-h-0 opacity-0'
          }`}
        >
